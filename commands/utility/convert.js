...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (conversionType === "usd_to_robux") {
      ...
      rateDescription = `${amount} USD is approximately ${Math.round(result)} Robux at a rate of ${customRate} USD per Robux.`;
    } else if (conversionType === "robux_to_usd") {
      result = amount * customRate;
      if (applyTax) {
        result *= 0.7;
      }
      rateDescription = `${amount} Robux is approximately $${result.toFixed(2)} USD at a rate of ${customRate} USD per Robux.`;
    }
